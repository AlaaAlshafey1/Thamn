...
<div class="card p-3" style="max-width:500px;margin:auto;">
    ...
    <form action="{{ isset($color) ? route('colors.update', $color->id) : route('colors.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="group" class="form-control" required value="{{ old('group', $color->group ?? '') }}">
        </div>
        ...
        <div class="mb-3">
            <label>القيمة</label>
            <input type="color" name="value" class="form-control form-control-color" required value="{{ old('value', $color->value ?? '#000000') }}">
        </div>
