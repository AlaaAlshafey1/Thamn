...
<div class="container">
    ...
    <div class="content">
        <h2>مرحباً أ. {{ $user->first_name }}،</h2>
        ...
        <div class="btn-box">
            <a href="{{ route('declaration.show', $token) }}" class="btn">عرض وتوقيع الوثيقة الآن</a>
        </div>

        <p style="font-size: 14px; color: #777;">إذا كنت تواجه صعوبة في الضغط على الزر، يمكنك نسخ هذا الرابط ولصقه في متصفحك:<br>
        <a href="{{ route('declaration.show', $token) }}" style="color:#c9933a; word-break: break-all;">{{ route('declaration.show', $token) }}</a></p>
    </div>
